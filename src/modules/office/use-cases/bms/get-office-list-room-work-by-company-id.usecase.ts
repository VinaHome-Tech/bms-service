...
@Injectable()
export class GetOfficeListRoomWorkByCompanyIdUseCase {
    constructor(private readonly repo: BmsOfficeRepository) { }

    async execute(companyId: string) {
        const result = await this.repo.findRoomWorkByCompanyId(companyId);
        return BmsOfficeMapper.toResponseRoomWorkList(result);
    }
} 